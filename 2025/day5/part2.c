...
int main() {
    ...
    for (int k = 0; k < i; k++) {
        // use two pointers to mark the beginning and end of a simulated merge

        //check if range_ends[k] overlaps with any range_starts[j] where j > k
        sum += range_ends[k] - range_starts[k] + 1;

        //k += current_end pointer
    }
